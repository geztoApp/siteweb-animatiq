...
// "https://animatiq.ch,https://www.animatiq.ch". Falls back to "*" (any
// origin) only if left unset — set this in production.
...
// --- Abuse limits ---------------------------------------------------------
// Every /chat call costs real money (OpenAI usage). These caps bound both
...
// Simple and sufficient for a single-process Node server. Resets on restart,
// which is fine for this use case (abuse limits, not billing-grade accounting).
...
// Periodically forget IPs with no recent activity so the map doesn't grow forever.
setInterval(() => {
  const now = Date.now();
...
// Reads the body while enforcing a byte-size cap, aborting the connection
// early if a caller tries to send an oversized payload.
const readBody = (req) =>
  new Promise((resolve, reject) => {
    ...
    req.on("data", (chunk) => {
      ...
      if (bytes > MAX_BODY_BYTES) {
        // Stop buffering further chunks, but let the stream finish normally
        // so we can still send a clean 413 response instead of resetting
        // the connection (which would leave the client with no answer at all).
        tooLarge = true;
        data = "";
        return;
      }
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === "/chat" && req.method === "POST") {
    ...
    if (!OPENAI_API_KEY) {
      sendJson(res, 500, { error: "OPENAI_API_KEY n'est pas configurée sur le serveur." }, req);
      return;
    }
    ...
    if (!validateMessages(messages)) {
      sendJson(res, 400, { error: "Conversation invalide ou trop longue." }, req);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        sendJson(res, 502, { error: "Erreur de l'API OpenAI." }, req);
        return;
      }
      ...
      const toolCall =
        choice && choice.message && choice.message.tool_calls && choice.message.tool_calls[0];
      ...
      sendJson(res, 200, { done: false, reply }, req);
    } catch (err) {
      const timedOut = err && err.name === "AbortError";
      sendJson(res, 502, { error: timedOut ? "L'assistant met trop de temps à répondre." : "Impossible de joindre OpenAI." }, req);
    }
    return;
  }
